...
def firstTimeBorrowerAge(df=None):
    """
    Format data for avg. age of first  time borrower
    """
    ...
    line = go.Scatter(x=x,y=y, line = dict(color = ('rgb(34, 167, 240)')), name='Average Age')
    data = [line]

    drawLineGraphs(data,title, xaxis, yaxis)

def firstTimeBorrowerGender(df=None):
    """
    Format data for gender breakdown of first time borrower
    """
...
def firstTimeBorrowerIncome(df=None):
    """
    Format data for avg. income of first  time borrower
    """

    if not df:
        df = getFile()

    df_inc = pd.read_csv(os.path.join(os.path.dirname(__file__),'data/input/census/income_data.csv'))
    ...
    line1 = go.Scatter(x=x,y=y1,line = dict(color = ('rgb(34, 167, 240)')),mode = 'lines+markers', name='Avg. Borrower Annual Income (in $)')
    line2 = go.Scatter(x=x,y=y2,line = dict(color = ('rgb(34, 167, 240)'), dash = 'dot'), name='Income Growth Rate (in %)', yaxis='y2')
    line3 = go.Scatter(x=x2, y=y3,line = dict(color = ('rgb(248,23,23)')), mode = 'lines+markers', name='Median National Income (Household)')
...
def firstTimeBorrowerEthnicity(df=None):
    """
    Format data for breakdown age of ethnicity for time borrower
    """

    if not df:
        df = getFile()

    df_eth = pd.read_csv(os.path.join(os.path.dirname(__file__),'data/input/census/demographic_data.csv'))
    ...
    line1 = go.Scatter(x=x,y=y1,line = dict(color = ('rgb(255, 211, 42)')),mode = 'lines+markers', name='American Indian or Alaskan Native')
    line2 = go.Scatter(x=x,y=y2,line = dict(color = ('rgb(15, 188, 249)')),mode = 'lines+markers', name='Asian')
    line3 = go.Scatter(x=x,y=y3,line = dict(color = ('rgb(5, 196, 107)')),mode = 'lines+markers', name='Black or African American')
    line4 = go.Scatter(x=x,y=y4,line = dict(color = ('rgb(245, 59, 87)')),mode = 'lines+markers', name='Native Hawaiian or Other Pacific Islander')
    line5 = go.Scatter(x=x,y=y5,line = dict(color = ('rgb(128, 142, 155)')),mode = 'lines+markers', name='White')

    line01 = go.Scatter(x=x1,y=y03,line = dict(color = ('rgb(255, 211, 42)'), dash = 'dot'),mode = 'lines+markers', name='US Census - American Indian or Alaskan Native')
    line02 = go.Scatter(x=x1,y=y04,line = dict(color = ('rgb(15, 188, 249)'), dash = 'dot'),mode = 'lines+markers', name='US Census - Asian')
    line03 = go.Scatter(x=x1,y=y02,line = dict(color = ('rgb(5, 196, 107)'), dash = 'dot'),mode = 'lines+markers', name='US Census - Black or African American')
    line04 = go.Scatter(x=x1,y=y05,line = dict(color = ('rgb(245, 59, 87)'), dash = 'dot'),mode = 'lines+markers', name='US Census - Native Hawaiian or Other Pacific Islander')
    line05 = go.Scatter(x=x1,y=y01,line = dict(color = ('rgb(128, 142, 155)'), dash = 'dot'), mode = 'lines+markers',name='US Census - White')
...
def drawLineGraphs(data, title, xaxis, yaxis, dual=False):
    """
    Process DataFrame date to output line graphs
    """

    plotly.tools.set_credentials_file(username=os.environ.get('USER'), api_key=os.environ.get('api_key'))
    if dual:
        layout = dict(title=title, xaxis={'title':xaxis}, yaxis={'title':yaxis}, yaxis2={'side':'right','overlaying':'y'})
    else:    
...
def executeFlow():
    """
    Execute entire anlysis workflow
    """
